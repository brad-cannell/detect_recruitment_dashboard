...
summarise(
  ...
  mutate(`% Change Between Two Weeks Ago and Last Week` = (((Totals-lag(Totals)) / Totals) * 100) %>%
           round(1) %>%
           paste0("%")) %>%
  filter(`% Change Between Two Weeks Ago and Last Week` != "NA%") %>%
  select(`% Change Between Two Weeks Ago and Last Week`)

valueBox(pct_change, icon = "fa-phone")
```
